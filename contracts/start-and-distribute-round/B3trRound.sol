    ...
    ReentrancyGuard
{
    ...
    function initialize(
        ...
        xAllocationVoting = IXAllocationVotingGovernor(
            _xAllocationVotingAddress
        );
     ...
     */
    function getAllXAppsForRound(
        uint256 roundId
    ) public view returns (bytes32[] memory) {
        // Get eligible apps from the XAllocationVoting contract
        bytes32[] memory eligibleApps = xAllocationVoting.getAppIdsOfRound(
            roundId
        );
     ...
     */
    function getUnclaimedXAppsForRound(
        uint256 roundId
    ) public view returns (bytes32[] memory) {
     ...
     */
    function getUnclaimedXAppsForPreviousRound()
        public
        view
        ...
        returns (bytes32[] memory appIds, uint256[] memory claimableAmounts)
    {
        ...
        for (uint256 i = 0; i < unclaimedApps.length; i++) {
            (uint256 totalAmount, , , ) = xAllocationPool.claimableAmount(
                roundId,
                unclaimedApps[i]
            );
     ...
     */
    function getUnclaimedXAppsWithNonZeroAmounts(
        uint256 roundId
    ) public view returns (bytes32[] memory) {
        bytes32[] memory unclaimedApps = getUnclaimedXAppsForRound(roundId);
     ...
     */
    function getUnclaimedXAppsWithNonZeroAmountsForPreviousRound()
        public
        view
        returns (bytes32[] memory)
    {
