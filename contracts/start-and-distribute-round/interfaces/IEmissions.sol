...
interface IEmissions {
    ...
    event MaxVote2EarnDecayUpdated(
        uint256 indexed newDecay,
        uint256 indexed oldDecay
    );
    ...
    event Vote2EarnDecayPeriodUpdated(
        uint256 indexed newPeriod,
        uint256 indexed oldPeriod
    );

    event Vote2EarnDecayUpdated(
        uint256 indexed newDecay,
        uint256 indexed oldDecay
    );

    event XAllocationsAddressUpdated(
        address indexed newAddress,
        address indexed oldAddress
    );

    event XAllocationsDecayPeriodUpdated(
        uint256 indexed newPeriod,
        uint256 indexed oldPeriod
    );

    event XAllocationsDecayUpdated(
        uint256 indexed newDecay,
        uint256 indexed oldDecay
    );

    event XAllocationsGovernorAddressUpdated(
        address indexed newAddress,
        address indexed oldAddress
    );
    ...
    function emissions(
        ...
        view
        returns (
            uint256 xAllocationsAmount,
            uint256 vote2EarnAmount,
