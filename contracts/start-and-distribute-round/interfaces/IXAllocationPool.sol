...
interface IXAllocationPool {
     ...
     */
    function isQuadraticFundingDisabledForRound(
        uint256 roundId
     ...
     */
    function isQuadraticFundingDisabledForCurrentRound()
        external
        view
        returns (bool);
    ...
    /**
     * @dev Check if an app has claimed its rewards for a given round
     * @param roundId The round ID
     * @param appId The ID of the app
     * @return true if the app has claimed its rewards for the given round
     */
    function claimed(
        uint256 roundId,
        bytes32 appId
